...
// On main this runs against the shared e2e webServer (playwright.config.ts); both default and the
// E2E_BASE_URL/APP_PASSCODE env overrides are honored so it travels across group worktrees.
...
test.describe.serial("Ask & Configure — authenticated flows", () => {
  ...
  test("settings: base currency saves and persists across a reload", async () => {
    await page.goto("/settings");
    // Shared e2e DB on main: pick a target different from the current value so Save isn't disabled
    // (Save is disabled when draft === saved), regardless of what earlier specs left behind.
    ...
    await page.reload();
    await expect(page.locator("strong")).toHaveText(target);
  });
